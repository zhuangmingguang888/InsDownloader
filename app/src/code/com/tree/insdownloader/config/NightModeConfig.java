...
public class NightModeConfig {
    ...
    public static final String IS_SYSTEM_MODE = "Is_System_Mode";

    private boolean mIsNightMode;
    private boolean mIsSystemMode;
    ...
    public void setSystemMode(Context context, boolean isSystemMode) {
        if (mSharedPreference == null) {
            mSharedPreference = context.getSharedPreferences(NIGHT_MODE, context.MODE_PRIVATE);
        }
        mEditor = mSharedPreference.edit();

        mEditor.putBoolean(IS_SYSTEM_MODE, isSystemMode);
        mEditor.commit();
    }

    public boolean getSystemMode(Context context) {

        if (mSharedPreference == null) {
            mSharedPreference = context.getSharedPreferences(NIGHT_MODE, context.MODE_PRIVATE);
        }
        mIsSystemMode = mSharedPreference.getBoolean(IS_SYSTEM_MODE, false);
        return mIsSystemMode;
    }
